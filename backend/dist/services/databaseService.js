...
class DatabaseService {
    ...
    async getAudiobookWithTracks(audiobookId) {
    ...
    async addTrackToAudiobook(audiobookId, trackId) {
        try {
            // Get next position
            const positionResult = await this.getQuery('SELECT COALESCE(MAX(position), 0) + 1 as next_position FROM audiobook_tracks WHERE audiobook_id = ?', [audiobookId]);
    ...
    async removeTrackFromAudiobook(audiobookId, trackId) {
        try {
            const result = await this.runQuery('DELETE FROM audiobook_tracks WHERE audiobook_id = ? AND track_id = ?', [audiobookId, trackId]);
            if (result.changes > 0) {
                await this.updateAudiobookStats(audiobookId);
                return true;
            }
    ...
    async reorderAudiobookTracks(audiobookId, trackIds) {
        try {
            await this.ensureReady();
            // Update position for each track
            for (let i = 0; i < trackIds.length; i++) {
                await this.runQuery('UPDATE audiobook_tracks SET position = ? WHERE audiobook_id = ? AND track_id = ?', [i, audiobookId, trackIds[i]]);
            }
            return true;
        }
        catch (error) {
    ...
    async updateAudiobookStats(audiobookId) {
        const stats = await this.getQuery(`
      SELECT
        COUNT(*) as track_count,
        COALESCE(SUM(t.duration), 0) as total_duration
      ...
      WHERE at.audiobook_id = ?
    `, [audiobookId]);
        await this.runQuery('UPDATE audiobooks SET track_count = ?, duration = ?, updated_at = ? WHERE id = ?', [stats.track_count, stats.total_duration, new Date().toISOString(), audiobookId]);
    }
    // RFID Card methods
